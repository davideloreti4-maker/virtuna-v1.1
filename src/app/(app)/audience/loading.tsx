 ...
 * shape-matched skeleton with no shift when the client tree hydrates. Uses the
 * shared Skeleton primitive (warm shimmer + reduced-motion guard).
 */
    ...
    <div className="mx-auto w-full max-w-[880px] px-4 pb-24 pt-6 lg:px-6">
      ...
      <div className="rounded-2xl bg-white/[0.02] px-2 pb-2 pt-3">
        ...
        <div className="flex flex-col divide-y divide-white/[0.045]">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex items-center gap-4 px-3.5 py-4">
              <Skeleton className="h-4 w-32 flex-1 rounded" />
              <Skeleton className="h-3 w-40 rounded" />
              <Skeleton className="h-[7px] w-28 rounded" />
            </div>
          ))}
        </div>
      </div>
