...
export function CameraOverlay({ activePreset, onSelect }: CameraOverlayProps) {
  return (
    <div
      className="fixed top-4 left-[60px] z-[100] flex items-center gap-0.5 rounded-[10px] border border-white/[0.06] px-1.5 py-1 shadow-[rgba(0,0,0,0.4)_0_4px_16px_0] sm:left-1/2 sm:-translate-x-1/2"
      style={{
    ...
    >
      {PRESET_ORDER.map((key) => (
        ...
        >
          ...
          <Kbd size="sm" className="hidden sm:inline-flex opacity-50">{PRESET_KEYS[key]}</Kbd>
        </button>
      ))}
      <div className="hidden sm:block mx-1 h-4 w-px bg-white/[0.06]" aria-hidden="true" />
      <button
        aria-label="Reset view"
        aria-keyshortcuts="R"
        title="Press R to switch view"
        onClick={() => onSelect('overview')}
        className="hidden sm:flex items-center gap-1 rounded-[6px] px-2 py-1 text-xs text-foreground/70 transition-colors hover:bg-white/[0.06] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#FF7F50]"
      >
        <span>Reset</span>
