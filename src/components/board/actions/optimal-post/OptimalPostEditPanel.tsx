...
interface Props {
  ...
  analysisId: string;
  /** Called after a successful save or reset so the host can collapse the panel. */
  onDone: () => void;
}
...
/**
 * Inline editor for the optimal posting window. Extracted from the former
 * OptimalPostEditSheet so the controls live INSIDE the Actions frame instead of
 * a Sheet drawer. All save/reset/keyboard/telemetry behaviour is preserved.
 */
export function OptimalPostEditPanel({ currentWindow, originalWindow, analysisId, onDone }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Mutation error visible via mutation.isError; keep the panel open.
    }
  }
  ...
  return (
    <div
      className="mt-2 flex flex-col gap-3 rounded-[8px] border border-white/[0.06] bg-white/[0.02] p-3"
      data-testid="optimal-post-edit-panel"
    >
      <div>
        <div className="mb-2 text-[10px] font-medium uppercase tracking-wide text-white/55">
        ...
        >
          {DAY_LABELS.map((d) => (
            <GlassPill key={d} size="sm" active={selectedDay === d} onClick={() => setSelectedDay(d)}>
              {d}
            </GlassPill>
          ))}
        </div>
      ...
      <div className="flex gap-3">
        <label className="flex flex-1 flex-col gap-1">
          ...
          <select
            ...
            className="h-[42px] rounded-lg border border-white/[0.06] bg-white/[0.05] px-2 text-xs text-white/85"
          >
            {hours.map((h) => (
      ...
      <button
        ...
        onClick={handleSave}
        className="h-[42px] w-full rounded-lg border border-white/[0.06] bg-transparent text-xs text-white/85 transition-colors duration-150 hover:bg-white/[0.1] disabled:cursor-not-allowed disabled:opacity-40"
      >
