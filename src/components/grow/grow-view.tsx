...
/**
 * GrowView — the GROW hub's "Monetize" tab body (shell-less): account+audience-grounded
 * monetization guidance as CARDS, not a chat engine (The Room owns the thread):
 * monetization readiness · pre-tested offer ideas · a pricing/funnel map. Every action is
 * the one Seam-4 handoff. The hub (grow-hub.tsx) owns the radial shell, header, and tabs.
 *
        ...
        <div className="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap sm:items-stretch">
          {data.funnel.map((step, i) => (
            <Fragment key={step.label}>
              ...
              >
                <span className="font-mono text-[8px] uppercase tracking-[0.08em] text-foreground-muted">
                  Step {i + 1}
                </span>
                <span className={cn("text-[12px] font-medium", step.filled ? "text-foreground" : "text-foreground-muted")}>
                  {step.label}
                </span>
                <span className="font-mono text-[9px] text-foreground-muted">{step.sub}</span>
              </div>
              {i < data.funnel.length - 1 && (
                <span aria-hidden className="hidden shrink-0 items-center text-border-hover sm:flex">
