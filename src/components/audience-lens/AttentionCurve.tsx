...
/**
 * AttentionCurve — Sapient's "PREDICTED ATTENTION · SMOOTHED, SAME PLAYHEAD" block, restyled to
 * the Room's document grammar (hairline section, one kicker, no box). A smoothed line of modeled
 * attention across the clip, peak dots on the crests, a playhead line at the current second, and a
 * modeled "hold" number top-right (labelled MODELED so it never reads as the room's real "N of 10
 * stopped"). The area fill is cream — the Room's "with you" pole — not sage (dosage pass). See
 * `attention-curve.ts`; renders nothing when empty (grounded-only).
 */
...
export function AttentionCurve({ curve, tSec, durationS }: { curve: Curve; tSec: number; durationS: number }) {
  ...
  return (
    <section className="mt-4 border-t border-[var(--color-border)] pt-3" data-testid="brain-attention-curve">
      <p className="mb-1.5 text-[10px] font-medium uppercase tracking-[0.12em] text-[var(--color-foreground-muted)]">
        Predicted attention
      </p>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="mt-2 h-[72px] w-full" aria-hidden>
        <path d={area} fill="rgba(236,231,222,0.08)" />
        ...
        {peaks.map((p) => (
          <circle key={p.i} cx={xOf(p.i, n)} cy={yOf(p.v)} r={1.1} fill="var(--color-foreground)" stroke="var(--color-background)" strokeWidth={0.5} vectorEffect="non-scaling-stroke" />
        ))}
      </svg>
