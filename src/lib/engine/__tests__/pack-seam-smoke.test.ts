 ...
 * [0,100], `engine_version === "3.22.0"`) — NOT exact values. Combined with git,
 ...
 *  T-01-CP: `engine_version === "3.22.0"` on the result AND `ENGINE_VERSION ===
 *    "3.22.0"` — catches an accidental version bump (Pitfall 2).
...
function expectStructurallyValid(result: PredictionResult): void {
  ...
  expect(result.engine_version).toBe("3.22.0");
}
...
describe("pack-seam smoke + structural gate (PACK-04, D-03/D-04) — BLOCKING", () => {
  ...
  it("ENGINE_VERSION is exactly '3.22.0' (T-01-CP — no accidental bump)", () => {
