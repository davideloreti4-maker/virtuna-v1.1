...
/**
 * Coerce small-model (omni-flash) TYPE sloppiness before Zod. Flash sometimes emits `null`
 * for required non-empty rationale strings (e.g. cta_segment.rationale → the observed read
 * failure), failing the WHOLE parse on one prose field. Fills only those required-`.min(1)`
 * strings with a neutral placeholder — fabricates NO score/perception (D-R1: rationale is
 * secondary prose, Apollo is the judge). No-op on clean omni-plus output. Critical-field
 * drift detection still runs after, so a genuinely-empty perception field still triggers retry.
 */
...
export async function analyzeVideoWithOmni(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      const coerced = coerceOmniRead(parsed); // salvage flash null'd required-rationale strings
