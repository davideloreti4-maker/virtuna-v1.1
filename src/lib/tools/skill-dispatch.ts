...
/** Args the MODEL supplies per skill call (extracted from the conversation). */
export interface SkillToolArgs {
  /** The subject to generate for. */
  topic: string;
...
// ─── The registry — the generators (share one input shape) ───────────────────
...
const DISPATCH_SYSTEM_PROMPT =
  ...
  "answer conversationally OR run a content skill by calling a tool — generate_ideas, generate_hooks, " +
  "write_script. Call a tool ONLY when the creator asks you to make that kind of content; extract the " +
  "topic from their message and the conversation so far. If they are just talking, asking strategy, or " +
  "thinking out loud, answer directly — do NOT call a tool they didn't ask for. After a skill runs, its " +
  "cards are already shown to the creator; add ONE short line pointing at what you made and a natural " +
  "next step. Never invent card content in text — the cards carry it.";
...
export async function runSkillDispatch(
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    for (const call of calls) {
      ...
      let args: SkillToolArgs = { topic: "" };
      try {
        const parsed = JSON.parse(call.function?.arguments ?? "{}");
        args = { topic: String(parsed.topic ?? "").trim(), anchor: parsed.anchor };
      } catch {
        args = { topic: "" };
      }

      // Unknown tool, empty topic, or over the paid leash → a tool result the model relays; no run.
      if (!skill) {
      ...
      if (!args.topic) {
        toolCalls.push({ name: skill.name, args, ran: false, note: "no topic" });
        messages.push({ role: "tool", tool_call_id: call.id, content: JSON.stringify({ error: "no topic provided" }) });
        continue;
      }
