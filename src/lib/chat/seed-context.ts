...
export function buildChatSystemContext(row: AnalysisRow, scope: ChatScope = null): string {
  ...
  const lines: string[] = [
    ...
    `- When grounded in a specific finding, you MAY cite a corpus section as §N (e.g. §1 for`,
    `  hook guidance). Use sparingly — only for key claims, at most 1-2 citations per answer.`,
    ...
    ``,
    `CORPUS SECTION MAP (use for §citations):`,
