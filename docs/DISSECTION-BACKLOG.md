...
| R2 | 🟡→✅ | **RESOLVED (verified 2026-06-26, T1.1 fold-in).** The double-count was the OLD headline = one Apollo call graded twice (`behavioral_score` = avg of Apollo's component_scores; `apollo_score` = same call's composite). Now the video headline is `0.5·apollo_score + 0.5·fold_audience_score` (`aggregator.ts:889`) — two GENUINELY INDEPENDENT signals (expert read × the 10-archetype audience sim), no double-count. The text/tiktok fallback `behavioral·w + apollo·w` (`:891`) is the INTENTIONAL no-fold proxy (byte-identical to pre-T1.1 text mode — there's no audience sim in text mode). Documented in-code at `aggregator.ts:852-855`. | `aggregator.ts:848-897` | RESOLVED (verified) |
| R3 | 🟡 | 0.5/0.5 video blend asserted, never calibrated | `aggregator.ts` | OPEN (post-launch A/B) |
| R4 | 🟢→✅ | **RESOLVED (verified 2026-06-26, F43 / 01-05).** The dead signals (ml/rule/trend/platform_fit/gemini) are removed from the blend and now emit HONEST NULLS, not fake constants: `rule_score:null trend_score:null ml_score:null` (`:1170-1174`), `platform_fit:null` (`:1253`), `audio_fingerprint:null` (`:1183`), `score_weights` dead keys pinned to 0 (`:1229`). `ruleResult`/`trendEnrichment` are no longer used for OUTPUT — they survive ONLY to feed the confidence `signal` heuristic (`:313-314`, legitimate, not dead). Honest-null passthroughs kept intentionally (F43 prunes fake CONSTANTS, not honest absence) with null-lock test coverage. | `aggregator.ts:1166-1253` | RESOLVED (verified) |
