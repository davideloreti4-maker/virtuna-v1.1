...
4. **Post-publish loop — ⏸ DEFERRED TO MILESTONE END (owner call 2026-07-03g).** Do NOT wire mid-milestone. The *write* path is real + wireable (paste-URL → `useOutcomeSignature`), but the *read* side the loop renders is **blocked**: the receipt scalar ("we said 7") is never persisted (`predicted_vector`/`realized_vector` are 6-dim normalized SHARE vectors, sum≈1 — `signature.ts` normalizes the count away; the stop-of-10 lives on `CardReaction` at sim-gen, not in `outcome_signatures`), and the accuracy % exists nowhere (`confidence-gate.ts` emits recalibration proposals, not a user %). Wiring real receipts = **inventing a metric**, which is the still-OPEN contract **§6.1** ("don't design it twice"). Also empty for the test user (0 outcome rows → renders empty; happy path unverifiable without seeded rows + a scrapeable TikTok URL). **At the very end, gate it: EITHER wire it** (needs an owner metric-definition call on the receipt-scalar + accuracy %, a confirmed surface-vs-engine ownership split on §6.1, and seeded outcome data) **OR remove the loop section for launch** (ship without it rather than ship a fabricated number). Until then the section stays as-is (mock, clearly commented).
...
| **The loop** | receipts "we said 7 · you got 7.2" + accuracy-rising | **✅ our surface** | **⏸ DEFERRED TO MILESTONE END (§4.4) — wire-or-remove gate. Read side blocked (scalar not persisted + accuracy % undefined + §6.1 OPEN); stays mock until the end call.** |
