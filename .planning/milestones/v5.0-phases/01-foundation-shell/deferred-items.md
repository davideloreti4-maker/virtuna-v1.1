...
- **Re-confirmed during 01-04:** Same 12 errors, same files (`engine/**/__tests__`, `board/verdict/__tests__/fixtures`); none overlap 01-04's diff (`middleware.ts`, `auth/callback/route.ts`, `middleware.landing.test.ts` all type-check with 0 errors). Still pre-existing engine-test debt, not introduced by the landing repoint.
