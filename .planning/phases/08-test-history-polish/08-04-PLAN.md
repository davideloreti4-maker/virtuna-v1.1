...
must_haves:
  truths:
    ...
    - "v0 MCP visual comparison completed for all new UI components"
  artifacts:
...
<task type="auto">
  <name>Task 4: v0 MCP visual comparison with societies.io</name>
  <files>None (visual verification only)</files>
  <action>
**MANDATORY: Use v0 MCP for pixel-perfect visual verification.**

This task uses v0 MCP to compare our implementation against societies.io reference.

1. Take screenshots of the following components:
   - Sidebar with test history list (items, icons, three-dot menu)
   - Delete confirmation modal
   - Legend pills in dashboard top bar
   - View selector dropdown with role level colors

2. Use v0 MCP tool (mcp__v0__v0_generate_from_image) to:
   - Upload screenshot of societies.io sidebar with test history
   - Ask v0 to analyze and compare with our implementation
   - Get specific feedback on spacing, typography, colors, hover states

3. Apply any polish fixes v0 identifies:
   - History item padding/margins
   - Icon sizes and alignment
   - Color accuracy (zinc-800, indigo-500, etc.)
   - Delete modal styling
   - Legend pill sizing and spacing

4. Iterate until v0 confirms visual match or discrepancies are documented.

**v0 MCP Query Template:**
```
Compare this implementation screenshot with societies.io reference.
Focus on:
- Test history list item layout (icon, text, score, menu)
- Delete confirmation modal styling
- Legend pills appearance and spacing
- View selector dropdown design

Identify specific pixel-level discrepancies and provide CSS fixes.
```

Document any intentional deviations in SUMMARY.md.
  </action>
  <verify>
    - v0 MCP analysis completed for all new UI components
    - Visual discrepancies identified and addressed
    - Any remaining differences documented with justification
  </verify>
  <done>UI components match societies.io reference per v0 MCP analysis</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Complete test history and polish features with v0 MCP verification:
...
**Visual Polish (v0 MCP verified):**
16. Compare sidebar history styling with societies.io - should match closely
17. Delete modal should match app's modal styling pattern
18. Legend pills should be properly sized and spaced
  </how-to-verify>
...
- [ ] v0 MCP visual comparison completed
- [ ] UI polish applied based on v0 feedback
</verification>
