...
<task type="auto">
  ...
  <action>
**Use v0 MCP first** to generate the CardActionMenu matching societies.io reference:

**v0 Query:** "Create a dropdown menu component for card actions with three options: Edit (pencil icon), Refresh (refresh icon), and Delete (trash icon, red text). The menu should:
- Trigger from a vertical 3-dot icon button
- Dark theme with zinc-800 background, zinc-700 border
- Rounded-lg corners, subtle shadow
- Items have icons on left, text on right
- Hover state: bg-zinc-700
- Delete item has red-400 text color
- Uses Radix UI DropdownMenu for accessibility
- Menu appears aligned to end (right side)"

Then implement the component using v0's output, adapting as needed:
