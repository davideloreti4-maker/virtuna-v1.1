...
### Research-confirmed decisions (added 2026-06-29 after RESEARCH.md surfaced premise corrections)
- **D-07: General skill-menu entries reuse existing flows + direct submit.** RESEARCH found Profile/Simulate/Predict are NOT in the `SKILLS`/`ToolId` array today — they're reached via card-chain CTAs + the evidence drop, so D-01's "tag them `modes:['general']`" requires defining per-skill submit behavior. **Confirmed:** Profile → opens the evidence-drop affordance (not a topic submit); Simulate/Predict → require a *selected General audience*, then submit the draft/scenario through the existing `/api/tools/{simulate,predict}` routes (`audienceId = selectedAudience.id`). True verb access from the front door, reuses all P5/P6 plumbing, gated per §16.4 asymmetry (Simulate/Predict pick Audience first). **Rejected:** launcher-only (menu merely navigates into the card-chain — weaker UX-02 delivery, no real skill-submit). The byte-identical Socials submit path stays untouched.
- **D-08: From-description Build path hops to the existing `/audience/new` calibration form.** D-02's "no surface hop" governs the *everyday picker*; the `+ Build an audience → From a description` path is a deliberate one-time setup action where reusing the full `/audience/new` `AudienceForm` (pre-set `mode:'general'`, returning to the composer with the new SIM selected) is acceptable and avoids duplicating the calibration UI. **Evidence path** (`profile-runner`) and **template path** (`audience-repo` clone) stay in-composer. **Rejected:** embedding a slim in-composer calibration mini-form (more work, partial duplication of `/audience/new`).

### Research-confirmed corrections (do NOT act on the stale premises)
- **A1 — the "retired `AudienceChip`" is stale.** `composer.tsx` no longer imports `audience-chip.tsx`; the LIVE in-composer picker is the `AudiencePresence` switcher (`audience-presence.tsx` L307–359). **D-02 enhances that switcher** (add `groupAudiences()` mode sections + `+ Build an audience`); resurrecting the chip would create two competing pickers. Treat the chip's 286 lines as a *markup reference only*.
- **D-06 "live" bar resolved to REUSE.** "Reacts live as you draft" is satisfied by generalizing the existing focus/explicit-ask reactor (AbortController throttle) to General — NOT a new per-keystroke debounce loop. CONTEXT D-06's own wording ("reusing the existing mechanism," "keep the existing Socials debounce/throttle") points at reuse; keystroke-live is a possible follow-up, out of P7.
