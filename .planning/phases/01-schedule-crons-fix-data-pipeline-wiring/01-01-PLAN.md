...
**Step 3 — Verify no evaluation_tier-specific casts exist:**
- Check `src/lib/engine/rules.ts` for any cast specifically working around a missing `evaluation_tier` field (e.g., casting a single property access through `as any` or `as unknown`).
- If such a cast exists, remove it now that the type includes `evaluation_tier`.
- **IMPORTANT:** The existing `as unknown as Rule[]` cast on line ~119 of rules.ts is a data-shape cast (casting the full Supabase response array to the `Rule[]` type). This cast is UNRELATED to evaluation_tier — leave it in place.
...
2. `grep -c "as unknown as Rule" src/lib/engine/rules.ts` returns 1 (the data-shape cast is still present — this is correct and expected)
3. No evaluation_tier-specific `as any` or `as unknown` workarounds exist in rules.ts (grep for `evaluation_tier.*as` returns no matches)
4. `pnpm build` exits with code 0
  </verify>
  <done>evaluation_tier column is in migrations AND reflected in TypeScript types. No evaluation_tier-specific type-cast workarounds in rules.ts (the existing data-shape `as unknown as Rule[]` cast is unrelated and remains). Build passes.</done>
</task>
