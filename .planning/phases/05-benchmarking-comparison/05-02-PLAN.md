...
must_haves:
  ...
  artifacts:
    - path: "src/app/(app)/competitors/page.tsx"
      provides: "Video query extended with posted_at field"
      contains: "posted_at"
    - path: "src/components/competitors/competitor-card.tsx"
      provides: "CompetitorCardData.videos type extended with posted_at"
      contains: "posted_at: string | null"
...
Output: 4 modified files -- video data extended with `posted_at`, enhanced table with sorting + dashboard header with compare link.
</objective>
...
<task type="auto">
  <name>Task 1: Add posted_at to video data flow for cadence computation</name>
  <files>
    src/app/(app)/competitors/page.tsx
    src/components/competitors/competitor-card.tsx
  </files>
  <action>
**Fix the video data pipeline so `computePostingCadence` receives the `posted_at` field it requires.**

Currently, `competitors/page.tsx` fetches videos with `.select("competitor_id, views, likes, comments, shares")` -- missing `posted_at`. The `CompetitorCardData.videos` type in `competitor-card.tsx` also lacks `posted_at`. Without this field, `computePostingCadence(competitor.videos)` will always return `null` because it filters on `v.posted_at !== null`.

1. **`src/app/(app)/competitors/page.tsx`**:
   - Change the video select query from:
     `.select("competitor_id, views, likes, comments, shares")`
     to:
     `.select("competitor_id, views, likes, comments, shares, posted_at")`
   - Update the `videosMap` type to include `posted_at`:
     ```
     const videosMap: Record<string, { views: number | null; likes: number | null; comments: number | null; shares: number | null; posted_at: string | null }[]> = {};
     ```
   - Update the video grouping loop to include `posted_at` in pushed objects:
     ```
     arr.push({ views: v.views, likes: v.likes, comments: v.comments, shares: v.shares, posted_at: v.posted_at });
     ```

2. **`src/components/competitors/competitor-card.tsx`**:
   - Extend the `CompetitorCardData.videos` type to include `posted_at`:
     ```
     videos: {
       views: number | null;
       likes: number | null;
     ...
     ```
   - The `CompetitorCard` component itself doesn't need to display cadence, so no rendering changes needed -- this is purely a type extension so downstream consumers (CompetitorTable) can call `computePostingCadence` on the same video data.
  </action>
  <verify>
- `npx tsc --noEmit` compiles without errors
- Check that the video query in page.tsx includes `posted_at` in the select string
- Check that `CompetitorCardData.videos` type includes `posted_at: string | null`
  </verify>
  <done>
- Video data flow includes `posted_at` from Supabase query through to component props
- `computePostingCadence(competitor.videos)` will receive dated videos and produce real cadence values instead of always returning null
  </done>
</task>

<task type="auto">
  <name>Task 2: Sortable column headers for CompetitorTable</name>
...
<task type="auto">
  <name>Task 3: Add Compare link to dashboard header</name>
