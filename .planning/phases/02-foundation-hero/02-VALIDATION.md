...
| 02-02-03 | 02 | 1 | HERO-06 (invariants) | T-2-05 | Vitest enforces mobile<desktop, percentage range, animation window, easing boundaries | unit | `test -f src/components/landing/__tests__/behavioral-hero-constants.test.ts && [ "$(grep -c 'it(' src/components/landing/__tests__/behavioral-hero-constants.test.ts)" -eq 4 ] && pnpm test -- behavioral-hero-constants` | ✅ | ✅ green |
