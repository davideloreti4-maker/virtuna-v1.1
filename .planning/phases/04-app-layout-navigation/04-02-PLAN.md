...
<task type="auto">
  <name>Task 1: Query v0 for Sidebar and SidebarNavItem components</name>
  <files>n/a (design generation)</files>
  <action>
Use the v0 MCP tool to generate pixel-perfect sidebar components.

**Query v0 with:**
1. Reference screenshot: .reference/app/_assets/dashboard-main.png (if supported, otherwise describe visually)
2. Design specifications from .reference/app/sidebar.md:
   - Width: 248px fixed, height: 100vh
   - Background: #0A0A0A, border-right: 1px solid #27272A
   - Padding: 16px, flex column layout
   - Header row: Logo (stylized "∧"), collapse button
   - Section labels: uppercase, zinc-500, text-xs, tracking-wide
   - Dropdown buttons: full width, zinc-900 bg, zinc-800 border, rounded-lg, px-4 py-3
   - Bottom nav items: flex between, zinc-400 text, 18px icons, hover white
   - Version text: centered, zinc-600, text-xs

**Prompt for v0:**
"Create a dark-themed sidebar component for a dashboard app. Fixed width 248px, full height. Structure from top to bottom: (1) Header with stylized logo mark and collapse button icon, (2) 'Current Society' label with dropdown trigger button showing 'Zurich Founders', (3) 'Current View' label with dropdown trigger showing 'Country', (4) 'Create a new test' action row with plus icon, (5) Spacer, (6) Separator line, (7) Bottom nav: Manage plan, Leave Feedback, Product Guide, Log Out - each with icon on right. Use zinc color palette, bg-[#0A0A0A]."

**Also request:**
- SidebarNavItem reusable component for the bottom nav items
- Use Lucide icons (ChevronDown, Plus, CreditCard, MessageSquare, BookOpen, LogOut)

**Adaptation notes:**
- Adapt v0 output to use project's `cn()` utility from `@/lib/utils`
- Use existing design tokens where applicable
- Ensure "use client" directive for interactive components
  </action>
  <verify>v0 returns component code for Sidebar and SidebarNavItem</verify>
  <done>v0-generated code available for adaptation into project components</done>
</task>

<task type="auto">
  <name>Task 2: Create SidebarNavItem component</name>
  <files>src/components/app/sidebar-nav-item.tsx</files>
  <action>
Adapt v0 output to create a reusable nav item component for sidebar bottom menu items.
...
Merge any improved patterns from v0 output while maintaining project conventions.
  </action>
...
<task type="auto">
  <name>Task 3: Create Sidebar component</name>
  <files>
    src/components/app/sidebar.tsx
    src/components/app/index.ts
  </files>
  <action>
Adapt v0 output to create the main Sidebar component matching .reference/app/sidebar.md specs.
...
<task type="auto">
  <name>Task 4: Integrate Sidebar into app layout</name>
  <files>
    src/app/(app)/layout.tsx
    src/app/(app)/dashboard/page.tsx
  </files>
