...
<task type="auto">
  <name>Task 1: Query v0 for FilterPill and ContextBar components</name>
  <files>n/a (design generation)</files>
  <action>
Use the v0 MCP tool to generate pixel-perfect filter pill and context bar components.

**Query v0 with design specs from .reference/app/dashboard.md and .reference/app/view-selector.md:**

**Prompt for v0:**
"Create dark-themed filter pill components and a context bar for a network visualization dashboard.

1. **FilterPill component:**
   - Rounded-full pill shape
   - Background: zinc-900/50, border zinc-800 (inactive) or zinc-600 (active)
   - Left side: 10px colored dot (variable color prop)
   - Right side: label text (zinc-400 inactive, white active)
   - px-4 py-1.5, text-sm
   - Hover: text-white
   - Clickable (toggle active state)

2. **FilterPillGroup component:**
   - Flex row with gap-2
   - Contains 4 pills with colors:
     - Executive Level: #6366F1 (indigo)
     - Mid Level: #EC4899 (pink)
     - Senior Level: #10B981 (emerald)
     - Entry Level: #F97316 (orange)
   - All active by default

3. **ContextBar component:**
   - Single pill showing location (e.g., 'Switzerland')
   - bg-zinc-800/80, border border-zinc-700, rounded-full
   - px-4 py-2
   - Optional colored dot + label text
   - Positioned at top-left of visualization area

Use zinc color palette on dark #0A0A0A background."

**Reference screenshot:** .reference/app/_assets/dashboard-main.png (shows pills at top with role level colors)

**Adaptation notes:**
- Adapt to use project's `cn()` utility
- Ensure "use client" directive for interactive FilterPill
  </action>
  <verify>v0 returns component code for FilterPill, FilterPillGroup, and ContextBar</verify>
  <done>v0-generated component code available for adaptation</done>
</task>

<task type="auto">
  <name>Task 2: Create NetworkVisualization component</name>
...
<task type="auto">
  <name>Task 3: Create FilterPill and ContextBar components</name>
  <files>
    src/components/app/filter-pills.tsx
    src/components/app/context-bar.tsx
  </files>
  <action>
Adapt v0 output to create FilterPill and ContextBar components.
...
Styling from research and v0 output:
- Rounded-full pill shape
...
<task type="auto">
  <name>Task 4: Assemble Dashboard page</name>
  <files>
    src/app/(app)/dashboard/page.tsx
    src/components/app/index.ts
  </files>
