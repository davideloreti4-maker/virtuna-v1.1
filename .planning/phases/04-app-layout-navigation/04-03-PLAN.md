...
<task type="auto">
  <name>Task 1: Query v0 for SocietySelector modal component</name>
  <files>n/a (design generation)</files>
  <action>
Use the v0 MCP tool to generate a pixel-perfect society selector modal.

**Query v0 with design specs from .reference/app/society-selector.md:**

**Prompt for v0:**
"Create a dark-themed modal dialog for selecting a 'society' in a dashboard app. The modal should:

1. **Container:** bg-zinc-900, border border-zinc-800, rounded-xl, p-6, min-width 600px, centered overlay with backdrop blur

2. **Two sections:**
   - 'Personal Societies' header (text-sm font-semibold white) with info icon
   - Grid of society cards (2 columns) with dashed borders (zinc-700), each showing: optional badge ('Setup' in orange), platform icon, society name, description
   - Include 'Create a Personal Society' card with plus icon

   - Separator line (border-t zinc-800 my-6)

   - 'Target Societies' header (same style)
   - Grid (3 columns) with: 'Create Target Society' card (dashed), existing society cards (solid border zinc-800, hover zinc-600)
   - Cards show: 'Custom' or 'Example' badge, icon, name, description, three-dot menu

3. **Selection behavior:** Clicking a card selects it (ring-2 ring-indigo-500 border), closes modal

4. **Close button:** X icon top-right, ESC key support

Use Radix Dialog primitives. Use zinc color palette with #18181B backgrounds."

**Adaptation notes:**
- Adapt to use project's `cn()` utility
- Use Radix Dialog (already installed per research)
- Ensure "use client" directive
  </action>
  <verify>v0 returns component code for SocietySelector modal</verify>
  <done>v0-generated modal code available for adaptation</done>
</task>

<task type="auto">
  <name>Task 2: Query v0 for ViewSelector dropdown component</name>
  <files>n/a (design generation)</files>
  <action>
Use the v0 MCP tool to generate a pixel-perfect view selector dropdown.

**Query v0 with design specs from .reference/app/view-selector.md:**

**Prompt for v0:**
"Create a dark-themed dropdown menu for selecting a 'view' in a dashboard app. Structure:

1. **Trigger button:** Full width, bg-zinc-900, border zinc-800, rounded-lg, px-4 py-3, shows current selection with chevron icon

2. **Dropdown content:** bg-zinc-900, border zinc-800, rounded-lg, py-2, min-width 160px, shadow-lg

3. **Header label:** 'VIEWS' in uppercase, zinc-500, text-xs, tracking-wide, px-4 py-2

4. **Menu items:**
   - Options: Country, City, Generation, Role Level, Sector, Role Area
   - Styling: px-4 py-2.5, text-sm, zinc-200 text
   - Hover: bg-zinc-800
   - Selected: white text (optionally with checkmark)

5. **Behavior:** Click outside or ESC closes, selecting item updates trigger text

Use Radix DropdownMenu primitives. Match dark zinc theme."

**Adaptation notes:**
- Adapt to use project's `cn()` utility
- Use Radix DropdownMenu (already installed per research)
- Ensure "use client" directive
  </action>
  <verify>v0 returns component code for ViewSelector dropdown</verify>
  <done>v0-generated dropdown code available for adaptation</done>
</task>

<task type="auto">
  <name>Task 3: Create Society Selector modal</name>
  <files>src/components/app/society-selector.tsx</files>
  <action>
Adapt v0 output to create Society Selector using Radix Dialog, matching .reference/app/society-selector.md.
...
<task type="auto">
  <name>Task 4: Create View Selector dropdown</name>
  <files>src/components/app/view-selector.tsx</files>
  <action>
Adapt v0 output to create View Selector using Radix DropdownMenu, matching .reference/app/view-selector.md.
...
<task type="auto">
  <name>Task 5: Integrate selectors into Sidebar</name>
  <files>
    src/components/app/sidebar.tsx
    src/components/app/index.ts
  </files>
