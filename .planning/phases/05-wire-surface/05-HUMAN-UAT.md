...
- **#R11-follower-scrape (NEW, layer 2 of 2 — still open):** even with the handle threaded, the R11 *range* needs `creator_profiles.tiktok_followers` populated, and that column is ONLY filled by the background scrape fired from `/api/profile` PATCH (route.ts:132). The ONBOARDING handle step writes `tiktok_handle` directly via the Supabase client and does NOT trigger that scrape, so `tiktok_followers` stays null for onboarding-only users → R11 range still won't render for them. Action (next session): trigger the follower scrape on the onboarding handle write too (or backfill on first analyze). Until then R11 renders only for users who set their handle via the profile/settings flow. Compute + threading are now correct; this is the last gate. To verify the positive range live: set a handle via Settings (fires scrape), wait for `tiktok_followers`, then run one analysis.
