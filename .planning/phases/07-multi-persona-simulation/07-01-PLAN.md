...
**(1a) Export `tryUrlHost` from `wave0/prompts.ts` (Phase-7-specific check #12 — reuse existing helper, do NOT duplicate).** Single-keyword edit:
- Read `src/lib/engine/wave0/prompts.ts:110` — current line: `function tryUrlHost(url: string): string {`
- Edit to: `export function tryUrlHost(url: string): string {`
- No other change to wave0/prompts.ts. The internal call site at `buildNicheUserMessage` line ~107 already references `tryUrlHost(w.url)` — works unchanged.
- This is the ONLY edit to wave0/prompts.ts in this plan; do NOT modify NICHE_SYSTEM_PROMPT or any other content.
...
// tryUrlHost is IMPORTED from ../wave0/prompts (see header) — do NOT redefine here.
// Phase 7 reuses the existing PROFILE-16 helper rather than duplicating it (Plan 07-01 Step 1a exports it).
```
...
```
  ...
  <acceptance_criteria>
    ...
    - File contains exact string `import { tryUrlHost } from "../wave0/prompts";` (B-3 reuse mandate)
    ...
    - `grep -c "^export function tryUrlHost" src/lib/engine/wave0/prompts.ts` returns 1 (B-3 — helper exported)
    - `grep -c "function tryUrlHost" src/lib/engine/wave3/persona-prompts.ts` returns 0 (B-3 — no duplicate definition; only the import)
    - `grep -c "import.*tryUrlHost.*wave0/prompts" src/lib/engine/wave3/persona-prompts.ts` returns 1 (B-3 — import line present)
    - `git diff src/lib/engine/wave0/prompts.ts` shows ONLY the `export` keyword added on line 110 (no other change to wave0/prompts.ts)
