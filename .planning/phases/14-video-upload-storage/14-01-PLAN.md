...
<task type="auto">
  ...
  <action>
    ...
    1. Import `createClient` from `@/lib/supabase/client` (browser client).

    2. Add new props to `VideoUploadProps`:
    ...
    3. When a file is selected via `handleFileSelection`, immediately start uploading:
    ...
    4. The component already has `uploadProgress` as a prop. Change this to internal state managed by the upload function. Remove `uploadProgress` from `VideoUploadProps` since the component now manages its own upload lifecycle.

    5. When user clicks remove (handleRemove): If there's an active upload, abort it. Reset all state.
