...
**Step 3: Generate CreateSocietyModal via v0**
...
**Step 4: Generate LeaveFeedbackModal via v0**
Call `mcp__v0__v0_generate_ui` with a prompt including:
...
- Brand bible context: same dark theme tokens
- Requirements:
  - Same Dialog replacement pattern as CreateSocietyModal
  ...
  - Add dirty-form confirmation (same pattern as CreateSocietyModal)
...
**Step 5: Integrate v0 outputs**
Write v0's generated code for each file, then manually adjust:
- Fix import paths: Dialog from `@/components/ui/dialog`, GlassTextarea from `@/components/primitives/GlassTextarea`, GlassInput from `@/components/primitives/GlassInput`, Button from `@/components/ui/button`, `z` from `zod`
- CreateSocietyModal: re-wire society store connection (`useSocietyStore`), ensure `handleSubmit` business logic is intact, verify back button works in multi-step flow
- LeaveFeedbackModal: re-wire settings store connection (`useSettingsStore`, `_isHydrated`), ensure `displayName`/`displayEmail` pre-fill logic works, verify success state rendering is intact
- Both: ensure `isDirty` + `showDiscardConfirm` pattern intercepts `onOpenChange` correctly (test: Dialog must NOT close when dirty, confirmation must appear instead)

**Step 6: Verify**
Run `npx tsc --noEmit`. Grep for `"@radix-ui/react-dialog"` in both files — must return zero. Grep for `DialogContent`, `GlassTextarea`, `GlassInput`, `Button` — must find them. Grep for hardcoded colors — must return zero.
  </action>
  <verify>
