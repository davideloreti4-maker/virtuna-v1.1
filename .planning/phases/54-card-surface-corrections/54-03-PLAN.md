...
must_haves:
  truths:
    ...
    - "GlassInput.tsx re-exports Input from ui/input.tsx for backward compatibility"
    - "GlassTextarea.tsx re-exports Textarea from ui/textarea.tsx for backward compatibility"
    - "video-card.tsx imports Card from ui/card instead of GlassCard from primitives"
    - "primitives/GlassCard.tsx is simplified to use Card from ui/ instead of GlassPanel+GradientGlow"
...
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/54-card-surface-corrections/54-01-SUMMARY.md
@.planning/phases/54-card-surface-corrections/54-02-SUMMARY.md
...
**video-card.tsx:**
- Read the file first to check what GlassCard props are used
- Change `import { GlassCard } from "@/components/primitives"` to `import { Card } from "@/components/ui/card"`
- Replace `<GlassCard` with `<Card` and adapt props:
  - If it uses `padding`, convert to className with p-*
  - If it uses `hover="lift"`, the Card now has hover built-in (hover:-translate-y-0.5)
  - If it uses `color`, `tinted`, `glow` -- these are Phase 55 concerns (GlassPanel effects). For now, just use Card which has the correct gradient bg + hover states
  - Check carefully what props the video-card passes and adapt accordingly
