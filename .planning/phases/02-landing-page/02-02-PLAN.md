...
must_haves:
  truths:
    ...
    - "Hive demo renders 50 pre-computed nodes with progressive build animation"
    - "Hive demo renders smoothly on mobile without jank (Canvas 2D, no physics)"
  artifacts:
    ...
    - path: "src/components/hive-demo/hive-demo-canvas.tsx"
      provides: "Canvas renderer with RAF lifecycle tied to visibility"
...
Verify that `touchAction: "auto"` still exists in hive-demo-canvas.tsx.
  ...
  <done>
HiveDemoCanvas conditionally mounts/unmounts based on IntersectionObserver visibility. RAF loop only runs when the section is within 200px of the viewport. Canvas placeholder prevents layout shift. Build passes.
  </done>
</task>
