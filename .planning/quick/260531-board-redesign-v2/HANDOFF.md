...
4. **Keyframe interaction** ✅ RESOLVED 2026-05-31. Investigated: there is **no dead affordance in the live product**. The "tap a frame to jump" copy existed ONLY in the dev `/board-preview` (faux `FilmCell`s) — softened it to "Fixes" to match live. Live Actions fix-row thumbnails (`ActionsContent.tsx:248` `KeyframeImage`) are static decorative context (the frame at the fix's moment); the row's real interaction is the headline `<button>` that expands the fix detail. The timestamp→filmstrip jump pattern already exists & works in `verdict/TopFixesList.tsx` (`onClick={setActivePreset('audience')}` camera-pan, gated on real video, aria-label "Jump to {time} on audience filmstrip"). NOTE: the board has **no seekable `<video>` player** (no `currentTime`/`seek`) — "jump" = camera preset to the filmstrip, not a video seek; a true video-seek is out of scope. OPTIONAL future enhancement: wire the live Actions thumbnails to the same `setActivePreset` jump (consistency) — not required (no copy promises it; row already has a tap action).
5. **Cleanup** — partial 2026-05-31. ✅ **Kit documented in `BRAND-BIBLE.md`** (new "Board Frame Kit" section after Components Overview — the law `Hero+tiles+tabs`, the coral accent rule, every primitive's API, keyframe units, responsive contract — the anti-re-divergence reference). ✅ **Dead exports deleted** from `verdict/verdict-derive.ts`: `deriveOneMove` + `OneMove` type + `formatTimestamp` (all orphaned — only test-referenced; `insights-section.tsx`'s `formatTimestamp` is a separate local fn) + their 4 tests removed. `one-move-banner`/`OneMoveBanner` already gone. Board suite 375/375, tsc + lint clean. ⏸️ **DEFERRED — env-gate/remove `/board-preview`:** a parallel session is actively building on the preview (added `CreatorRulebookCard`); gating/removing it now would disrupt their QA. Do this at **merge-time** (it's publicly routable under `(marketing)` — must 404 in prod or be deleted before merge to main).
