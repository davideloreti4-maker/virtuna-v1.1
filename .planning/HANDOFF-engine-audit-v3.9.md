...
- ⏳ **REMAINING:** T1.5 (degradation honesty — engine + UI, NEEDS ENGINE_VERSION bump; design fork open: nullable `overall_score` [~15 display-site ripple, DB already nullable] vs `analysis_unavailable` boolean flag [contained, like `anti_virality_gated`] — see the T1.5 AskUserQuestion options), T3.1 (KNOWLEDGE_CORE trim — needs A/B), T3.3, T3.4, T4.x. Note: T3.4/T4.3 are now partly moot post-T1.1 (selectWeights only governs the text-mode fallback). The legacy `simulation/EngagementRangeCard` still renders projected-views — separate non-board surface, left out of Tier-1 scope. NOTE: a background auto-commit hook on this branch injects junk "test/feat: changes" commits — commit your own work promptly with real messages (it grabbed f1111513's work as "test: changes"; was amended).
