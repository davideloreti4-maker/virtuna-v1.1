...
**Deferred (v6.1+):** in-thread monetization; brand-profile entity; RAG over creator history; desktop dense-instrument; compact-onboarding redesign + link-social prefill. *(**Scripts + Remix un-deferred into v6.0 Phase 6** on 2026-06-18 — combined; Remix revives `milestone/viral-remix` prior art; see `phases/05-open-chat-test-reframe/05-CONTEXT.md` D-00.)*
...
| v6.0 (2026-06-18): P5 re-scoped to "Studio Conversation Layer" + Scripts/Remix un-deferred into a combined P6 | New -- owner expanded P5 into the integrative "feels like one conversation" layer (progress, cards-in-chat, chat-to-refine, generic chain plumbing); Script+Remix have prior art + run the same Qwen pipeline as Test, so they earn their own phase now rather than v6.1. See `05-CONTEXT.md` D-00 |
