...
**Target features (v1):**
- **De-risk spikes (gate the build):** SIM-1 Flash text-fidelity (does a text-only fold predict pull? — gates inline scoring + self-judge); Knowledge-Core generative rebuild (the long pole); Remix-reuse scout of `milestone/viral-remix`.
...
**Locked constraints:** **SIM-1 Max scoring FROZEN at ENGINE_VERSION 3.19.0** — no changes to recorded-video scoring; new engine work is **additive only** (SIM-1 Flash text-mode + generation calls). Qwen-only. Rich UI via the **fixed numen-rework typed-renderer library** — NOT model-generated UI (craft trap), NOT plain text (loses the moat). Flat-warm visual system (THEME-06) is the design SSOT. Spike-gated: Flash text-fidelity must pass before committing the inline-scoring architecture. Discuss input (EXPLORATORY, walk through every section): `.planning/NUMEN-TOOLS-VISION.md`.
...
| SIM-1 Max scoring frozen 3.19.0; new engine work additive — SIM-1 Flash text-mode + generation (v6.0) | New -- don't touch proven video scoring; the new bet is text-mode personas + grounded generation |
