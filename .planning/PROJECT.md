...
**Current milestone:** **v7.0 Numen GSI** — active in `~/virtuna-numen-gsi/` (`milestone/numen-gsi`), started 2026-06-26. Generalize the socials-locked engine into a domain-blind synthetic-population simulator: engine/pack seam (Socials → Pack #1), pluggable scoring, `audiences` → general SIM, then the General surface (Profile / Simulate / Predict) with Profile-a-chat → Simulate-reply as the on-ramp wow. Creator core untouched; marketplace deferred. Phase 0 (signature substrate) already on `main`. Input vision: `.planning/NUMEN-GSI-VISION.md`. Requirements + roadmap defined via `/gsd-new-milestone`. **Phase 1 (Engine / Pack Seam) complete 2026-06-26** — engine goes domain-blind via the typed 7-field `DomainPack` interface; `SOCIALS_PACK` extracted as Pack #1 (identity-wraps the frozen `aggregateScores` fold + `runPredictionPipeline`); all 4 production call sites dispatch through `resolvePack("socials")`; creator output structurally unchanged (D-03 smoke gate green, ENGINE_VERSION 3.20.0); PACK-01..04 validated. **Phases 2–5 complete (through 2026-06-29):** trustworthy-SIM spike (verdict NO-GO→GO after dropping thinking-mode synth); General population + honesty layer (`audiences` → domain-agnostic SIM library, Validated/Directional badges, authorable success-criterion); input adapter (text/file/image → `Stimulus`, auto SIM-1 tier); and the **Profile → Simulate on-ramp wow** — drop a chat → forensic `profile-read` card (behavioral tells each bound to a verbatim evidence quote, honest `Directional` badge, no 0-100 score) → "Simulate a reply →" → `reaction-distribution`, all in ONE thread (PROF-01..04, SIMU-01..03; human-verified live). Next: Phase 6 (Predict verb).
